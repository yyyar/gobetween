...
type Iphash2Balancer struct {
	...
	/* ip str -> session */
	table map[string]*Iphash2Session
...
func NewIphash2Balancer(cfg config.BalanceConfig) interface{} {
	...
	b.duration, _ = time.ParseDuration(cfg.IpHash2SessionExpire)
...
func (b *Iphash2Balancer) Elect(context core.Context, backends []*core.Backend) (*core.Backend, error) {
	...
	if !ok {
		...
		b.table[context.Ip().String()] = &Iphash2Session{
			backend: backend,
			mutex:   sync.Mutex{},
